...
function znote_loginWebService.onStartup()
    Spdlog.info(" ")
	Spdlog.info("=============================")
	Spdlog.info("= Znote AAC loginWebService =")
	Spdlog.info("=============================")
    local configLua = {
    ...
    elseif configSQL.SERVER_NAME ~= configLua.SERVER_NAME then
        ...
        Spdlog.info("= Updated [SERVER_NAME] FROM [" .. configSQL.SERVER_NAME .. "] to [" .. configLua.SERVER_NAME .. "]")
    end
    ...
    elseif configSQL.IP ~= configLua.IP then
        ...
        Spdlog.info("= Updated [IP] FROM [" .. configSQL.IP .. "] to [" .. configLua.IP .. "]")
    end
    ...
    elseif configSQL.GAME_PORT ~= tostring(configLua.GAME_PORT) then
        ...
        Spdlog.info("= Updated [GAME_PORT] FROM [" .. configSQL.GAME_PORT .. "] to [" .. configLua.GAME_PORT .. "]")
    end
    if #inserts > 0 then
        db.query("INSERT INTO `znote_global_storage` (`key`,`value`) VALUES "..table.concat(inserts,',')..";")
        Spdlog.info("= Fixed " .. #inserts .. " missing configurations.")
    end
    Spdlog.info("=============================")
    Spdlog.info("= SERVER_NAME: " .. configLua.SERVER_NAME)
    Spdlog.info("= IP: " .. configLua.IP)
    Spdlog.info("= GAME_PORT: " .. configLua.GAME_PORT)
    Spdlog.info("=============================")
    Spdlog.info(" ")
end
