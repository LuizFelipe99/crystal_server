...
static CombatDamage applyElementalDamageBonus(const std::shared_ptr<Player> &attackerPlayer, CombatDamage damage) {
	if (!attackerPlayer) {
		return damage;
	}

	if (damage.primary.type != COMBAT_HEALING && damage.primary.value < 0) {
		const int32_t primaryBonus = attackerPlayer->getElementalDamageBonus(damage.primary.type);
		if (primaryBonus > 0) {
			const float primaryMultiplier = 1.0f + (primaryBonus / 100.0f);
			damage.primary.value = static_cast<int32_t>(damage.primary.value * primaryMultiplier);
		}
	}

	if (damage.secondary.type != COMBAT_HEALING && damage.secondary.value < 0) {
		const int32_t secondaryBonus = attackerPlayer->getElementalDamageBonus(damage.secondary.type);
		if (secondaryBonus > 0) {
			const float secondaryMultiplier = 1.0f + (secondaryBonus / 100.0f);
			damage.secondary.value = static_cast<int32_t>(damage.secondary.value * secondaryMultiplier);
		}
	}
...
void Combat::CombatHealthFunc(const std::shared_ptr<Creature> &caster, const std::shared_ptr<Creature> &target, const CombatParams &params, CombatDamage* data) {
	...
	if (attackerPlayer) {
		...
		damage = applyElementalDamageBonus(attackerPlayer, damage);
